// Seed data for the ~20 saved Reports and 5 Dashboards we ship out of the box.
// Both `prisma/seed.ts` and `scripts/seed-reports-dashboards.mjs` use this data
// so they stay in sync. Dates are computed at call time so each seed run uses
// a fresh "30 days ago", "today", "start of month" anchor.
...
export interface ReportSeed {
  ...
  sortBy: string | null;
  sortDir: "asc" | "desc";
  summarize: Array<{ field: string; kind: "count" | "sum" | "avg" }>;
  rowLimit: number;
}

export interface DashboardTileSeed {
  kind: "kpi" | "count" | "sum" | "bar" | "table" | "report";
  title: string;
  queryKey?: string;
  reportName?: string;
...
export function getReportSeeds(): ReportSeed[] {
  return [
    {
...
export function getDashboardSeeds(): DashboardSeed[] {
  return [
    {
      name: "Sales Overview",
