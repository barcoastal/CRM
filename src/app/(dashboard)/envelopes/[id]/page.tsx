  ...
  return (
    <div style={{ padding: 20 }}>
      <div style={{ marginBottom: 12, fontSize: 12, color: "#706e6b" }}>
        <Link href="/envelopes" style={{ color: "#0070d2" }}>Envelopes</Link>
        <span> / </span>
          ...
          <article style={card}>
            ...
            <div style={{ padding: 12 }}>
              ...
              <dl style={{ marginTop: 12, fontSize: 12 }}>
                ...
                {envelope.template && (
                  <Row label="Template" value={<Link href={`/templates/esign/${envelope.template.id}`} style={{ color: "#0070d2" }}>{envelope.template.name}</Link>} />
                )}
                {envelope.opportunity && (
                  <Row label="Opportunity" value={
                    <Link href={`/opportunities/${envelope.opportunity.id}`} style={{ color: "#0070d2" }}>
          ...
          <article style={{ ...card, marginTop: 12 }}>
            ...
            <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
              {envelope.events.length === 0 ? (
                <li style={{ padding: 12, fontSize: 12, color: "#706e6b" }}>No events.</li>
              ) : (
                envelope.events.map((ev) => (
                  <li key={ev.id} style={{ padding: "10px 12px", borderTop: "1px solid #f3f3f3", fontSize: 12 }}>
                    ...
                    <div style={{ color: "#706e6b" }}>{ev.createdAt.toLocaleString()}</div>
...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "100px 1fr", gap: 6, padding: "4px 0" }}>
      <dt style={{ color: "#706e6b" }}>{label}</dt>
      <dd style={{ margin: 0, color: "#080707", wordBreak: "break-word" }}>{value || <span style={{ color: "#aaa" }}>--</span>}</dd>
    </div>
  );
}

const card: React.CSSProperties = { background: "#fff", border: "1px solid #d8dde6", borderRadius: 4 };
