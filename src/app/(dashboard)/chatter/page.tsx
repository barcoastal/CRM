  ...
  return (
    <div style={{ padding: "20px 24px", maxWidth: 1080, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#080707", margin: 0 }}>
            Chatter
          </h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 20 }}>
        ...
        <aside>
          <div style={{ border: "1px solid #d8dde6", borderRadius: 6, background: "#fff", padding: 16 }}>
            <h3 style={{ fontSize: 12, fontWeight: 700, color: "#3e3e3c", textTransform: "uppercase", letterSpacing: 0.4, marginTop: 0, marginBottom: 10 }}>
              My Groups
            </h3>
            {myGroups.length === 0 && (
              <div style={{ fontSize: 12, color: "#706e6b" }}>
                You are not a member of any groups yet. <Link href="/chatter/groups" style={{ color: "#0070d2" }}>Browse groups</Link>.
              </div>
            )}
            <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 6 }}>
              {myGroups.map((g) => (
                <li key={g.id}>
                  <Link href={`/chatter/groups/${g.id}`} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 8px", borderRadius: 4, textDecoration: "none", color: "#080707", fontSize: 13 }}>
                    <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{g.name}</span>
                    <span style={{ fontSize: 11, color: "#706e6b" }}>{g._count.members}</span>
                  </Link>
                </li>
              ))}
            </ul>
