...
function direction(callType: string | null): { label: string; color: string } {
  ...
  return { label: "Outbound", color: "#0070d2" };
}
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16, flexWrap: "wrap" }}>
        ...
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, color: "#3e3e3c" }}>
        ...
        <span style={{ fontSize: 13, color: "#706e6b" }}>
          <strong style={{ color: "#080707" }}>{data?.onCall ?? 0}</strong> on call · {data?.totalAgents ?? 0} agents tracked
        </span>
        <Link href="/dialer" style={{ marginLeft: "auto", color: "#0070d2", fontSize: 13, fontWeight: 600, textDecoration: "none" }}>
          ← Back to dialer
        </Link>
      </div>
      ...
      {!error && calls.length === 0 && (
        <div style={{ color: "#706e6b", padding: 48, textAlign: "center", background: "#fff", border: "1px solid #d8dde6", borderRadius: 6 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 12 }}>
        {calls.map((c) => {
          ...
          return (
            <article key={c.five9UserId} style={{ background: "#fff", border: "1px solid #d8dde6", borderRadius: 6, padding: 14 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#fff", background: dir.color, padding: "2px 8px", borderRadius: 10 }}>
                  {dir.label}
                </span>
                <span style={{ fontFamily: "monospace", fontSize: 16, fontWeight: 700, color: "#080707" }}>{fmtDur(liveDur)}</span>
              </div>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#080707", marginBottom: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              ...
              {c.campaignId && c.campaignId !== "0" && (
                <div style={{ fontSize: 11, color: "#706e6b", marginTop: 4 }}>Campaign {c.campaignId}</div>
              )}
