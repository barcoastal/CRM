...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!view.isSystem && !view.isShared && view.ownerId !== r.session.userId) return NextResponse.json({ error: "Not found" }, { status: 404 });
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const d = parsed.data;
  const filterError=listViewFilterError(view.entity,d.filters??[]);
  if(filterError)return NextResponse.json({error:filterError},{status:400});
  const data: Record<string, unknown> = {};
  if (d.baseView !== undefined) data.baseView = d.baseView;
