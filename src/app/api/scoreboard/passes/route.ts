...
import { scoreboardPasses } from "@/lib/scoreboard";
...
export async function GET(request: Request) {
  ...
  try {
    return Response.json(await scoreboardPasses(cursor), { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
    console.error("[scoreboard] Pass feed failed", error);
    return Response.json({ error: "Live transfers are reconnecting." }, { status: 503 });
  }
}
