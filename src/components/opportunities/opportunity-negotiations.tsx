...
export function OpportunityNegotiations({ opportunityId, debts, opportunityName = "Opportunity", canEmail = false, senderEmail = "", emails = [], documents = [], canCreateOffer = false }: { opportunityId: string; debts: Debt[]; opportunityName?: string; canEmail?: boolean; senderEmail?: string; emails?: NegotiationEmailMessage[]; documents?: Document[]; canCreateOffer?: boolean }) {
  ...
  const debt = debts.find((item) => item.id === selectedId) ?? debts.find(item => negotiationStage(item.negotiationStatus, item.status) !== "Settled") ?? debts[0];
  ...
  return <div className="ng-workspace">
    ...
    <div className="ng-main">
      ...
      <div className="ng-tab-content" id="ng-panel-Documents" role="tabpanel" aria-labelledby="ng-tab-Documents" hidden={tab !== "Documents"}><div className="ng-panel-heading"><div><h3 className="ng-section-title">Related documents</h3><p>Source contract and related opportunity, account, and lead files.</p></div><a className="ng-button" href={`/opportunities/${opportunityId}`} target="_blank" rel="noopener noreferrer">Manage files ↗</a></div><div className="ng-document-filters"><input type="search" aria-label="Search related documents" placeholder="Search by file name or document type…" value={documentSearch} onChange={event => setDocumentSearch(event.target.value)} /><select aria-label="Document source" value={documentSource} onChange={event => setDocumentSource(event.target.value)}><option value="all">All sources</option><option value="contract">This debt’s source contract</option><option value="Opportunity">Opportunity</option><option value="Account">Account</option><option value="Lead">Lead</option></select><span>{filteredDocuments.length} of {relatedDocs.length} files</span></div><ul className="ng-doc-list">{filteredDocuments.map(doc => <li key={doc.id}><span className="ng-file-icon">▤</span><div><a href={`/api/opportunities/${opportunityId}/negotiation-documents/${doc.id}?view=1`} target="_blank" rel="noopener noreferrer">{doc.name}</a><small>{doc.id === debt.sourceDocument?.id ? "Source contract" : `${doc.origin || "Opportunity"} document`} · {doc.type.replace(/_/g, " ")} · {new Date(doc.createdAt).toLocaleDateString()}{doc.fileSize != null ? ` · ${Math.ceil(doc.fileSize / 1024)} KB` : ""}</small></div><a className="ng-button" href={`/api/opportunities/${opportunityId}/negotiation-documents/${doc.id}`}>Download</a></li>)}</ul>{!filteredDocuments.length && <p className="ng-empty">{relatedDocs.length ? "No documents match these filters." : "No related documents have been uploaded."}</p>}</div>
