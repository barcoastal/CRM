...
export function LeadListPicker({current,userId,views=LEAD_LIST_VIEWS}:{current:string;userId:string;views?:LeadListDefinition[]}) {
  ...
  useEffect(()=>{
    if(!navigating.current&&!searchParams.has('view')&&pinned!=='recent'){
  ...
  function select(value:string){
    ...
    setRequested(views.find(v=>v.value===value)?.label??'list');
    ...
    startTransition(()=>router.push(`/leads?${sp}`));setOpen(false);setSearch('');
  }
  ...
  return <div ref={ref} className="relative inline-flex items-center gap-3" onKeyDown={e=>{if(e.key==='Escape')setOpen(false);}}>
    {pending&&<div role="status" aria-live="polite" className="fixed bottom-6 left-1/2 z-[100] -translate-x-1/2 rounded border border-[#0176d3] bg-white px-5 py-3 text-sm text-[#0176d3] shadow-lg">Loading {requested}…</div>}
