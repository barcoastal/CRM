    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl p-5" style={{ boxShadow: "0 12px 40px rgba(19,27,46,0.06)" }}>
        <div className="flex items-center gap-3 flex-wrap">
          <div className="text-[14px] font-bold text-[#131b2e] flex-1">
            Members <span className="text-[#706e6b] font-normal">({props.memberCount} total)</span>
          </div>
          <input
            value={addLeadId}
      ...
      <div className="bg-white rounded-xl overflow-hidden" style={{ boxShadow: "0 12px 40px rgba(19,27,46,0.06)" }}>
        {props.members.length === 0 ? (
          <div className="text-[13px] text-[#706e6b] py-10 text-center">No members yet.</div>
        ) : (
            ...
            <tbody>
              {props.members.map((m) => (
                <tr key={m.id} className="border-b border-[#f2f3ff] last:border-b-0">
                  ...
                  <td className="px-3 py-2 text-[#706e6b]">{m.source}</td>
                  <td className="px-3 py-2 text-[#706e6b]">{new Date(m.addedAt).toLocaleString()}</td>
