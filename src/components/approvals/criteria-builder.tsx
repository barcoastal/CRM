...
export function CriteriaBuilder({
  ...
  return (
    <div className="space-y-2">
      {value.length === 0 && (
        <div className="text-[12px] text-[#706e6b] italic">No criteria. Any record will match.</div>
      )}
      {value.map((rule, idx) => {
        const valueDisabled = rule.operator === "isNull" || rule.operator === "isNotNull";
        return (
          <div key={idx} className="flex items-center gap-2">
            <span className="text-[11px] text-[#706e6b] w-4">{idx + 1}</span>
            <input
              value={rule.field}
              onChange={(e) => update(idx, { field: e.target.value })}
              placeholder="field (e.g. amount, totalDebt)"
              className="flex-1 px-3 py-1.5 border border-[#d8dde6] rounded text-[13px] outline-none focus:border-[#3052ff]"
            />
            <select
              value={rule.operator}
              onChange={(e) => update(idx, { operator: e.target.value })}
              className="px-2 py-1.5 border border-[#d8dde6] rounded text-[13px] outline-none focus:border-[#3052ff] bg-white"
            >
              {OPERATORS.map((op) => (
            ...
            <input
              ...
              className="flex-1 px-3 py-1.5 border border-[#d8dde6] rounded text-[13px] outline-none focus:border-[#3052ff] disabled:bg-[#f3f2f2]"
            />
            <button
              type="button"
              onClick={() => remove(idx)}
